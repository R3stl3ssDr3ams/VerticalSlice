...
## Milestone 3 Devlog
<img width="589" height="458" alt="Screenshot 2026-05-28 234427" src="https://github.com/user-attachments/assets/0dc391e6-386a-4359-98e3-46405ca1b241" />

1. The shader graph is very similar to the one seen in the W8 2C activity, as they are meant to reach very similar effects. When complete, any time the player executes an NPC, a muzzle flash post processing effect should play in order to cover in order to place emphasis on the action of killing one of your neighbors as well as making the transition from the NPC being present to their asset suddenly being destroyed more believable. It works quite simply by taking the texture of a muzzle flash image found on the web and multiplying it against the background, allowing them to blend together. A Sine Timer and Lerp node is used to create the effect of the muzzle flashing in and out. The idea is to have this effect run on a timer so that it only plays once. Currently, it plays over the intro sequence for demonstrative purposes.
