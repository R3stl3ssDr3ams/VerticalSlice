...
## Milestone 3 Devlog
<img width="589" height="458" alt="Screenshot 2026-05-28 234427" src="https://github.com/user-attachments/assets/60b5c92e-95c5-478b-ae43-6e8b8dbcf642" />

1. When complete, the shader graph should activate any time the player executes an NPC, coming in the form of a muzzle flash post processing effect should play in order to cover in order to place emphasis on the action of killing one of your neighbors as well as making the transition from the NPC being present to their asset suddenly being destroyed more believable. It works quite simply by taking the texture of a muzzle flash image found on the web and multiplying it against the background, allowing them to blend together. A Sine Timer and Lerp node is used to create the effect of the muzzle flashing in and out. The idea is to have this effect run on a timer so that it only plays once.
